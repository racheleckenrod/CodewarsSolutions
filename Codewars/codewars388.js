...
function peopleLeft(busStops){
    // console.log(busStops.reduce((acc,c) => Number(acc) + +c[1],0) , busStops.reduce((acc,c) => acc + +c[0],0))
    return (busStops.reduce((acc,c) => +acc + +c[0], 0)) - (busStops.reduce((acc,c) => +acc + +c[1], 0))
  }
console.log(peopleLeft([[3,0],[9,1],[4,10],[12,2],[6,1],[7,10]]))
