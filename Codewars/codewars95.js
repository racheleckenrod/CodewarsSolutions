...
// Pseudocode:
// this is a sum of arrays within an array. reduce then reduce again..?
// s = [days] days = [number of stairs ]

// s.reduce((acc, c) => acc + c, 0) , but must get the reduction of the variable first maybe will need to forEach the array first.

//  daily = s.forEach(item.reduce((acc, c) => acc + c, 0))
...
//     assert.strictEqual(stairsIn20(stairs), 54636040);54636040
//   })
// })
...
function stairsIn20(stairs){
    ...
    let daily = []
    console.log(stairs)
    daily = stairs.reduce((acc, c) => acc.concat(c), [])
     console.log(daily)
    let yearly = daily.reduce((acc, c) => acc + c, 0)
    twentyYears = 20 * yearly
