...
// PREP
// P: we are given three numbers. the first, g is the step or difference we are to look for in prime numbers, starting from  m and continuing to n. 
// R: we are to return the first pair of prime numbers with a space of "g" between them, starting at "m" and continuing to "n", if none found we are to return null
// E: see above
// P: we check the first number for prime, and if it is, we also check the number + g for prime, and if it also is, we return an array with the two numbers. else check the next number. I we get to the end without finding any pairs, return null.
