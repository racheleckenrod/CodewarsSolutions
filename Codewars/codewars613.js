...
function List(){
     ...
     this.countSpecDigits = function(integersList, digitsList){
        let nums = []
        console.log(integersList.join().split(digitsList[1]).length -1, integersList.join().split(''), digitsList[1],integersList.join().split(digitsList[1]))
        for(let i = 0; i < digitsList.length; i++){
            nums.push([digitsList[i], integersList.join().split(digitsList[i]).length -1])
        }
        return nums
    }
  ...
  console.log(l.countSpecDigits([-77, 8, -65, 56, 888, -79, 6666, 222],[1,8,4]))