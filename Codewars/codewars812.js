...
// PREP
// Parameters: we are given an array of at least four numbers, not zero, can be pos and negative
// Results: we are to return an array of numbers with the evens first then the odds.
// Evens are to be ascending, and odds decending
// ex:
// Pseudocode: first filter even and odds arrays, sort them (and deal with repeat numbers if needed) accordingly, then concatenate them


function menFromBoys(arr){
    let evens = new Set(arr.filter(item => item % 2 === 0).sort((a,b) => a - b))
    let odds = new Set(arr.filter(item => item % 2 !== 0).sort((a,b) => b - a))

    return [...evens, ...odds]
    //your code here
  }

  console.log(menFromBoys([20,20,33,33,50,34,43,46]))