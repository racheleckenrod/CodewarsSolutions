...
// first sort[2,3,5,9,10]
// then add 2 + 2+2+3 + 7+2+5 + 14+2+9 + 25+2+10 =85
// so we need two tallys... the total wait time and the customer wait time. 
// we could make a new array of each customers wait, then reduce it.
...
// PREP
// Parameters: we are given an array of numbers representing how long each customer must wai for their drink
// we are to return the total minimun wait time, adding 2 after each order (for cleaning time)

// first sort the array so that the smallest number is first.
