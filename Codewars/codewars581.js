...
// PREP
// Parameters:
// given a number n- the upper limit and d, the maximum difference between digits
// results- must be two digits at least, and all digits must be ascending 
// 
// Pseudocode:
// running into issues as I begin.. perhaps should deal with the numbers as strings and concatanate
// 
...
function selNumber(n, d) {
    ...
    //    first get all the  ascending numbers
...
//         for(let j = 0; j < x.length; j++){
//             // console.log(x[x.length-1] - x[x.length-2])
//             // if(j !== x.length - 1){
                
//                 if(x[j + 1] > x[j]){
//                     if(x[x.length-1] - x[x.length-2] <= d && x[x.length-1] - x[x.length-2] > 0){

//                         console.log(x.join(''), x[x.length-1] - x[x.length-2],"TEST" )
//                         arr.push(x.join(''))
